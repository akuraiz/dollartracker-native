...
const APP_VERSION = "4.0.0-alpha.10";
...
function initEvents() {
  ...
  window.addEventListener("resize", () => { applyDevicePerformanceMode(); updateNavPill(); updateVisualViewportLayout(); });
}



/* ---------------------------------------------------------------------------
   Native-ready visual viewport support
   Keeps the floating Save button above the iOS keyboard/accessory bar while
   preserving the same behavior in Safari, PWA, and future Capacitor builds.
--------------------------------------------------------------------------- */
let visualViewportBaseline = 0;

function isEditableControl(element) {
  if (!element || !(element instanceof HTMLElement)) return false;
  if (element.matches("input, textarea, [contenteditable='true']")) {
    const inputType = (element.getAttribute("type") || "text").toLowerCase();
    return !["button", "checkbox", "file", "radio", "range", "submit"].includes(inputType);
  }
  return false;
}
...
function updateVisualViewportLayout() {
  ...
  const focused = isEditableControl(document.activeElement);

  if (!visualViewportBaseline) {
    visualViewportBaseline = Math.max(viewportHeight, window.innerHeight || 0);
  }
  if (!focused) {
    visualViewportBaseline = Math.max(visualViewportBaseline, viewportHeight, window.innerHeight || 0);
  }

  const lostHeight = Math.max(0, visualViewportBaseline - viewportHeight);
  const keyboardOpen = focused && lostHeight > 110;
  const saveTop = Math.max(12, viewportTop + viewportHeight - 72);

  document.documentElement.style.setProperty("--visual-viewport-top", `${Math.round(viewportTop)}px`);
  document.documentElement.style.setProperty("--visual-viewport-height", `${Math.round(viewportHeight)}px`);
  document.documentElement.style.setProperty("--keyboard-inset", `${Math.round(lostHeight)}px`);
  document.documentElement.style.setProperty("--keyboard-save-top", `${Math.round(saveTop)}px`);
  document.body.classList.toggle("keyboard-open", keyboardOpen);
}

function initVisualViewportLayout() {
  const viewport = window.visualViewport;
  visualViewportBaseline = Math.max(viewport?.height || 0, window.innerHeight || 0);

  viewport?.addEventListener("resize", updateVisualViewportLayout, { passive: true });
  viewport?.addEventListener("scroll", updateVisualViewportLayout, { passive: true });
  window.addEventListener("orientationchange", () => {
    visualViewportBaseline = 0;
    window.setTimeout(updateVisualViewportLayout, 180);
  });
  document.addEventListener("focusin", event => {
    if (isEditableControl(event.target)) {
      visualViewportBaseline = Math.max(visualViewportBaseline, viewport?.height || 0, window.innerHeight || 0);
      window.requestAnimationFrame(updateVisualViewportLayout);
      window.setTimeout(updateVisualViewportLayout, 120);
    }
  });
